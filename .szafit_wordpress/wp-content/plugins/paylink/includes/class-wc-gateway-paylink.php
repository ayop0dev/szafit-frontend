...
//        $this->icon = apply_filters('woocommerce_paylink_icon', plugins_url('../assets/icon.min.png', __FILE__));
        $this->icon = apply_filters('woocommerce_paylink_icon', plugin_dir_url(__DIR__) . 'assets/icon.min.png');
...
//                    'isDigital' => $product->is_virtual(),
    ...
    public function get_invoice()
    {
        try {
            ...
            $endpoint = ''; // avoid undefined warning
            ...
            if ($order_status === 'paid') {
                $return_url = $this->_get_order_return_url($order);
                if (!empty($return_url)) {
            ...
            $this->_postError(esc_html($this->fail_msg) . ', ' . $e->getMessage(), isset($endpoint) ? $endpoint : 'N/A', __FUNCTION__);
        } finally {
    ...
    /**
     * Resolve the order language for the Paylink API.
     *
     * @param WC_Order $order
     * @return string
     */
    private function _get_order_language(WC_Order $order): string
    {
        $language = sanitize_key((string) $order->get_meta('_szafit_language'));

        return in_array($language, ['ar', 'en'], true) ? $language : 'ar';
    }

    /**
     * Resolve the headless return URL for the order.
     *
     * @param WC_Order $order
     * @return string
     */
    private function _get_order_return_url(WC_Order $order): string
    {
        $return_url = trim((string) $order->get_meta('_szafit_return_url_base'));
        if (empty($return_url)) {
            return '';
        }

        $validated_url = wp_validate_redirect($return_url, '');
        if (empty($validated_url)) {
            return '';
        }

        return untrailingslashit($validated_url);
    }
